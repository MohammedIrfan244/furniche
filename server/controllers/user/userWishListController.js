...
const getUserWishList = async (req, res) => {
  const data = await WishList.findOne({ userId: req.user.id },{_id:0,userId:0,createdAt:0,updatedAt:0}).sort({ createdAt: -1 }).populate(
    "products",
    "name price image"
  );
  if (data) {
...
const addToWishList = async (req, res, next) => {
  ...
  const product=await Product.findOne({ _id: productId }, { name: 1, price: 1, image: 1});
  if (!productId) {
...
const removeFromWishList = async (req, res, next) => {
  const { id } = req.params;
  const item=await Product.findOne({ _id: id }, { name: 0, price: 0, image: 0, description: 0, rating: 0, original: 0, category: 0, review: 0 ,isDeleted:0});
