   ...
   <div className="flex flex-col sm:flex-row w-[100%]  mt-[3%]">
    ...
    <div className="mt-[3%] sm:mt-0 w-[100%] sm:w-[50%] flex flex-col justify-between min-h-[50vh] sm:px-[3%] sm:gap-0">
      ...
      <p className="text-xs">{product?.review}</p>
        ...
        <Link to={currentUser==null?'/login':'#'}><button onClick={currentUser!=null?()=>addCart(Id):null} className="border active:scale-95 bg-black text-xs text-[#F5F2E9] px-5 py-1 sm:py-2">{`${cartItems[Id]>0?"Added":"Add to Cart"}`}</button></Link>
    </div>
   </div>
