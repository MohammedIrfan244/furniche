        ...
        <div className="flex flex-row justify-between w-full bg-gray-100">
            <div className="w-1/4 bg-white border-r-2 border-gray-300 p-5 space-y-5 pt-10">
                <div className="h-36 flex justify-center items-center rounded-full mb-5 overflow-hidden w-36 border-2 border-gray-300">
                    <img src={user.avatar} alt="User avatar" onError={(e) => {
                ...
                <div className='flex justify-between'>
                    <button className='bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 text-xs rounded-lg' onClick={() => blockUser(userId, !user?.isBlocked)}>
                        {user?.isBlocked ? "Unblock" : "Block"}
                    </button>
                    <button className='bg-red-500 hover:bg-red-600 text-white px-3 py-1 text-xs rounded-lg' onClick={() => removeUser(user.id)}>
                        Delete
                    </button>
                    <Link className='bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 text-xs rounded-lg' to={'/adminpanel'}>
                        Go back
                    </Link>
                </div>
            </div>
            <div className='w-3/4 p-5'>
                <h1 className='mt-16 text-3xl mb-5 font-semibold'>ORDERS</h1>
                <div className="flex flex-col gap-5 w-full overflow-y-auto scrollbar-thin h-[90vh]">
                    {user.orders.length === 0 ? "Order is empty" :
